public class ReorderListTests
{
    private static ListNode? BuildList(int[] values)
    {
        ListNode? head = null;
        ListNode? tail = null;

        foreach (var v in values)
        {
            var node = new ListNode(v);
            if (head == null)
            {
                head = node;
            }
            else
            {
                tail!.next = node;
            }
            tail = node;
        }

        return head;
    }

    private static int[] ToArray(ListNode? head)
    {
        var values = new List<int>();
        while (head != null)
        {
            values.Add(head.val);
            head = head.next;
        }
        return values.ToArray();
    }

    [Theory]
    [InlineData(new int[] { 1, 2, 3, 4 }, new int[] { 1, 4, 2, 3 })]
    [InlineData(new int[] { 1, 2, 3, 4, 5 }, new int[] { 1, 5, 2, 4, 3 })]
    [InlineData(new int[] { 1 }, new int[] { 1 })]
    [InlineData(new int[] { 1, 2 }, new int[] { 1, 2 })]
    public void ReorderList_ReordersInPlace(int[] input, int[] expected)
    {
        var sol = new Solution();
        var head = BuildList(input);

        sol.ReorderList(head);

        Assert.Equal(expected, ToArray(head));
    }

    [Fact]
    public void ReorderList_EmptyList_DoesNotThrow()
    {
        var sol = new Solution();

        var exception = Record.Exception(() => sol.ReorderList(null));

        Assert.Null(exception);
    }
}
